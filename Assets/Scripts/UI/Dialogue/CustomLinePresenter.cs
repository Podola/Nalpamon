...
public sealed class CustomLinePresenter : DialoguePresenterBase
{
    ...
    /// finished presenting their lines. If a <see cref="CustomLinePresenter"/>
    ...
    /// </remarks>
    [Header("Automatically Advance Dialogue")]
    ...
    public override async YarnTask RunLineAsync(LocalizedLine line, LineCancellationToken token)
    {
        ...
        if (showCharacterNameInLineView)
        {
            ...
            {
                characterNameText.text = GetCharNameText(line.CharacterName);
            }
    ...
    private string? GetCharNameText(string? name)
    {
        if (name == null) return name;

        if (ChapterDataManager.Instance.NoteCharDict.TryGetValue(name, out NoteCharInfoSO info))
        {
            return LocalizeDataManager.GetText(info.Table, info.title);
        }
        else return name;
    }
}
